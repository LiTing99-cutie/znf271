...
file=paste0("/home/user/data2/lit/project/ZNF271/02-APA-1/analysis/stringtie_whole_genome_ref_based_all_1/stringtie.rpkm.",args[1],".txt")
rpkm <- fread(file)
...
p <- ggboxplot(ggplot.input,x="period.abbre.abbre",y="rpkm_proximal",color="period.abbre.abbre",outlier.shape = NA,width = 0.6,lwd=1,
               palette = c('#99d8c9','#005824'))+
  geom_jitter(size=1,color="gray40",alpha=1,position=position_jitter(width = 0.2,height = 0))+
...
p <- ggboxplot(ggplot.input,x="period.abbre.abbre",y="rpkm_distal",color="period.abbre.abbre",outlier.shape = NA,width = 0.6,lwd=1,
               palette = c('#99d8c9','#005824'))+
  geom_jitter(size=1,color="gray40",alpha=1,position=position_jitter(width = 0.2,height = 0))+
...
p <- ggboxplot(ggplot.input,x="period.abbre.abbre",y="pau",color="period.abbre.abbre",outlier.shape = NA,width = 0.6,lwd=1,
               palette = c('#99d8c9','#005824'))+
  geom_jitter(size=1,color="gray40",alpha=1,position=position_jitter(width = 0.2,height = 0))+
  labs(x="Developmental stage",y = "PA usage")+
  guides(color="none")
