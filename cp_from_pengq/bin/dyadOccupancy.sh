...
rm ./danpos_rst/pooled/*.wig;
samtools view $inputBam |grep -E "SA|XA" |cut -f1 >multAligName.txt
perl ~/bin/bam_filterByReadname.pl -i $inputBam -r multAligName.txt |samtools view -@ 10 -bSu - |samtools sort -@ 10 - out.sorted.uniq
...
samtools sort -@ 10 out.sorted.uniq.rmdup.bam  out.uniq.rmdup.nameSorted
#perl ~/bin/bam_insert_filter.pl -i out.uniq.rmdup.nameSorted.bam  -q 20 -n 10 | samtools view -@ 10 -bSu - | samtools sort -@ 10 -n - out.uniq.rmdup.filter.sortedName
#bamtools stats -insert -in out.uniq.rmdup.filter.sortedName.bam >out.uniq.rmdup.filter.sortedName.bamStats &
if [ $danpos ];then
    danpos.py dpos out.uniq.rmdup.nameSorted.bam -jd 147 -m 1 --extend 74 -o danpos_rst >danpos.log 2>danpos.err;
    awk -v OFS="\t" '{if($1!="chr"){print $1,$4-1,$4}}' danpos_rst/pooled/*positions.xls >danpos_rst/pooled/nucleosome_dyad.bed3;
    perl ~/bin/wigNormalize.pl -i danpos_rst/pooled/*.wig -c $chrSize >danpos_rst/pooled/NC_norm.wig
    wigToBigWig -clip  danpos_rst/pooled/NC_norm.wig $chrSize danpos_rst/pooled/NC_norm.bw 2> danpos_rst/pooled/wigToBigwig.log
    danpos.py profile ./danpos_rst/pooled/*.smooth.wig --genefile_paths ~/data/ce11/Caenorhabditis_elegans.WBcel235.87.gpe --genomic_sites TSS --flank_up 1000 --flank_dn 1000 >profile.log 2>profile.err
else
