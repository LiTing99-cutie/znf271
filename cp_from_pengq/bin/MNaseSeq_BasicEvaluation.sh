...
outDir=$PWD
perlScript=/home/pengq/bin
while getopts "hi:b:g:d:f:l:o:" OPTION
do
    case $OPTION in
        h) usage;;
        i) inputBam=$OPTARG;;
...
java -jar /home/pengq/anaconda3/envs/python2/share/picard-2.26.10-0/picard.jar CollectInsertSizeMetrics -I $inputBam -O insert_size_metrics.txt -H insert_size_histogram.pdf > log/collectInsertS.log 2> log/collectInsertS.err
echo "Calculate mismatch profile-----"
mismatch_profile.py -i $inputBam -l $readLen -o mismatchProfile 2>log/mismatchProfile.log &
echo "Calculate dinucleotide frequency across nucleosomes-----"
perl $perlScript/peReadsFragmentMid.pl -i $inputBam -f 147,147  >n147_mid.txt 
...
awk '{if($3 == "+"){print $2"\t"$4-1500"\t"$4+1501"\t"$2"_"$4"\t0\t"$3;}else{print $2"\t"$5-1501"\t"$5+1500"\t"$2"_"$5-1"\t0\t"$3}}' $gpeStructure |awk -v OFS="\t" '{if($2<0){print $1,"0",$3,$4,$5,$6}else{print $0}}'|sort|uniq >TSS_updn1.5k.bed6
...
sh $perlScript/TSSprofile_bwtool.sh -u 1500 -d 1500 -g $gpeStructure -b $occupancy -o TSS_profile.tsv
Rscript $perlScript/lines.R -i=TSS_profile.tsv -x="TSS relative position" -y="Normalized nucleosome occupancy" -c="blue" -o=TSS_profile.pdf 
...
#Rscript $perlScript/lines.R -i=exon_meta_profile.tsv -x="Exon relative position" -y="Normalized nucleosome occupancy" -c="blue" -o=exon_meta_profile.tsv.pdf
#bwtool agg 500:100 $exonBed $occupancy exon_up_profile.tsv -starts
#Rscript $perlScript/lines.R -i=exon_up_profile.tsv -x="Exon relative position" -y="Normalized nucleosome occupancy" -c="blue" -o=exon_up_profile.tsv.pdf
#bwtool agg 100:500 $exonBed $occupancy exon_down_profile.tsv -ends
#Rscript $perlScript/lines.R -i=exon_down_profile.tsv -x="Exon relative position" -y="Normalized nucleosome occupancy" -c="blue" -o=exon_down_profile.tsv.pdf
