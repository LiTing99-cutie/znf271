...
while(<$IN>){
    ...
    if(! defined $tag){
        ...
        if($name eq $tag){
            if($M_name_now eq "=" && (abs($insertS_now)>=$minS && abs($insertS_now)<=$maxS) && (abs($insert_first)>=$minS && abs($insert_first)<=$maxS) && ($mapQ_now>=$mapQ && $mapq_first>=$mapQ) && ($nm_now<=$nm && $nm_first<=$nm)){
                say $read;
                say $_;
            }
        }else{
...
Options:
    -i|--bam   FILE    The input bam file which should be sorted by read name.[Can be read from STDIN with header]
    -s|--size  STRING  The insert size range separated by comma.[default:50,300]
    		       The insert size is just extracted from the 9th column, so please ensure youknow its meanings in bam genereated by different aligers!
