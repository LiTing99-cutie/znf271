...
system.time(tmp <- parSapply(cl,head(gene_lst,5),GetPvalue,res=res))

stopCluster(cl)

tmp %>% apply(MARGIN = 2,unlist) %>% t() %>% data.frame()%>% rownames_to_column(.,"gene_name") %>% na.omit() -> res_f

# sapply(head(gene_lst,5),GetPvalue,res=res) %>% apply(MARGIN = 2,unlist) %>% t() %>% data.frame()%>%
