...
# read test data in 
s_1 <- fread("/home/user/data2/lit/project/ZNF271/02-APA-1/analysis/evo_compare/h/h.cnt.bed")
s_2 <- fread("/home/user/data2/lit/project/ZNF271/02-APA-1/analysis/evo_compare/m/m.cnt.bed")
...
p_adjust <- function(df){
  df$p_value_wc_adjust <- p.adjust(df$p_value_wc,"BH")
  df$p_value_ks_adjust <- p.adjust(df$p_value_ks,"BH")
  return(df)
}

res_1 <- p_adjust(res_1)
res_2 <- p_adjust(res_2)

# significant gene number and intersect number
n_sig <- function(df,p_1,p_2){
  p_1 <- enquo(p_1)
  p_2 <- enquo(p_2)
  filter(df,UQ(p_1)<0.05) %>% nrow() ->n_1
  filter(df,UQ(p_2)<0.05) %>% nrow() ->n_2
  filter(df,UQ(p_1)<0.05 & UQ(p_2)<0.05) %>% nrow() ->n_3
...
length(gene_lst)
print("per")
n_sig(res_1,p_value_wc,p_value_ks)
n_sig(res_1,p_value_wc_adjust,p_value_ks_adjust)
print("len")
n_sig(res_2,p_value_wc,p_value_ks)
n_sig(res_2,p_value_wc_adjust,p_value_ks_adjust)
