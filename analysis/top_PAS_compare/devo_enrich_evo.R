...
enrich_hrm <- filter(res_hrm,p_adjust<0.05) %>% select(GN_H)

test <- function(type_1,type_2,enrich,alternative){
  intersect(enrich$GN_H,type_1$gene_name) -> enrich_type_1
...
sapply(l, test,type_1=type_0,enrich=enrich_hrm,alternative="two.sided") -> diff_back_fisher_0
sapply(l, test,type_1=type_1,enrich=enrich_hrm,alternative="two.sided") -> diff_back_fisher_1
...
permutation <- function(type_1,type_2,enrich){
  ...
  for (i in 1:10000){
    ...
    intersect(tmp_1,enrich$GN_H) %>% length()-> n_1
    df <- rbind(df,n_1)
  }
  ...
  # overlap in fact
  intersect(type_2$gene_name,enrich$GN_H) %>% length() ->n
  # p_value
...
diff_back <- function(type_1){
  lapply(l, permutation,type_1=type_1,enrich=enrich_hrm) ->per_re
...
diff_back(type_0) -> diff_back_mc_0
diff_back(type_1) -> diff_back_mc_1
