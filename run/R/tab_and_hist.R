...
pdf("/home/user/data2/lit/project/ZNF271/02-APA/output/R/hist.pdf")
...
write.table(final.list[,c("gene_symbol","gene_id","gene_type","diff_pa_usage","diff_rpkm")],file = "/home/user/data2/lit/project/ZNF271/02-APA/output/final_list/diff.lst",
            quote = FALSE,row.names = FALSE,sep = '\t')

hist(as.data.frame(table(final.list$gene_type)),xlab = "PA usage after birth",ylab="Count",main=NULL,col = "#2b83ba")
p <- as.data.frame(table(final.list$gene_type)) %>% mutate(type=c("lncRNA","Coding","Pseudogene")) %>% 
